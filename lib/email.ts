...
export async function sendLeadEmail(
  ...
  const from = process.env.LEAD_FROM_EMAIL ?? 'LC Landscaping <onboarding@resend.dev>';
  ...
  const html =
    ...
    `<h2 style="font-family: Georgia, serif; font-weight: 500; letter-spacing: -0.02em;">New quote request</h2>` +
    ...
    row('Project', escape(PROJECT_TYPE_LABELS[data.projectType])) +
    `</table>` +
    ...
    `<hr style="border: 0; border-top: 1px solid #D4CFC3; margin: 20px 0;" />` +
    `<p style="font-size: 12px; color: #7A7D80;">Submitted ${new Date().toISOString()}<br/>IP ${escape(meta.ip ?? 'unknown')}<br/>UA ${escape(meta.userAgent ?? 'unknown')}</p>` +
    `</div>`;
...
function row(label: string, value: string) {
  return `<tr><td style="font-size:11px; text-transform:uppercase; letter-spacing:0.18em; color:#7A7D80; padding-right:16px; vertical-align:top;">${label}</td><td>${value}</td></tr>`;
}
