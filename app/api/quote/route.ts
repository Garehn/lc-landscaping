...
export async function POST(req: NextRequest) {
  ...
  try { body = await req.json(); }
    ...
    return NextResponse.json({ ok: false, errors: [{ message: 'Invalid JSON body' }] }, { status: 400 });
  }
  ...
  const result = await sendLeadEmail(parsed.data, { ip, userAgent: req.headers.get('user-agent') });
  if (!result.ok) {
